...
function loadArticles() {
  ...
  const mainStoryHTML = `
    <div class="main-story-content">
      <h1><a href="#" style="text-decoration: none; color: inherit;">${mainStoryArticle.title}</a></h1>
      <p class="summary">${mainStoryArticle.description}</p>
      <div class="byline"><strong>${mainStoryArticle.author}</strong> • ${mainStoryArticle.date}</div>
  ...
  const gridStoriesHTML = gridArticles.map(article => `
    <article class="story-card">
      ...
      <div class="byline"><strong>${article.author}</strong> • ${article.date}</div>
